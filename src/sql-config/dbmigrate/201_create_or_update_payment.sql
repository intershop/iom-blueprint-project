...
BEGIN
	...
	INSERT INTO oms."Shop2PaymentDefDO"
		("id",
		"active",
		"initOrderStateDefRef",
		"paymentDefRef",
		"shopPaymentName",
		"shopRef",
		"safePayment",
		"payment2ProcessStageDefRef",
		"useAutomaticAuthorizationExtension",
		"authizationDurationInDays",
		"timeForPaymentAllowed")
	SELECT nextval('oms."Shop2PaymentDefDO_id_seq"'),
		p_active,
		2001,     -- initOrderStateDefRef
		p_paymentdefref,
		p_shoppaymentname,
		p_shopref,
		FALSE,     --safePayment
		p_payment2ProcessStageDefRef, 
		FALSE,    --useAutomaticAuthorizationExtension
		NULL,
		NULL
	ON CONFLICT ("paymentDefRef", "shopRef")
		DO UPDATE SET
			active = EXCLUDED.active,
			"shopPaymentName" = EXCLUDED."shopPaymentName",
	...
	IF p_paymentproviderref IS NULL THEN
		DELETE FROM oms."Shop2PaymentProvider2PaymentDefDO"
			WHERE "shopRef" = p_shopref 
			AND "paymentDefRef" = p_paymentdefref;
	ELSE
		INSERT INTO oms."Shop2PaymentProvider2PaymentDefDO"(id, "shopRef", "paymentProviderRef", "paymentDefRef")
			 SELECT nextval('oms."Shop2PaymentProvider2PaymentDefDO_id_seq"'),
			 p_shopref,
			 p_paymentproviderref,
			 p_paymentdefref
		ON CONFLICT("shopRef", "paymentDefRef") 
			DO UPDATE SET 
				"paymentProviderRef" = EXCLUDED."paymentProviderRef";
	END IF;
